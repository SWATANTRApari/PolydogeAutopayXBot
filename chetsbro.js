...
const privateKey = '7c437651f57ecbc38be8c9858c75c75da27bf78cb5083d2e3beea5ac50841769'
let tokenAddress = '0x7ddD655aBB2Cd1B9Fc7A320378b25892dB90E33E'
let fromAddress = '0x294B1fc9db99d6ADAE709B4A03C35Be248E92D2E'
...
twiterhandle.on('text', async (ctx) => {
 ...
 adreshandle.on('text', async (ctx) => {
  ...
  await ctx.replyWithMarkdown(data.comment, { disable_web_page_preview: true,reply_markup: { inline_keyboard: [[{ text: "Done ☑️", callback_data: "☑️ Done" },{ text: "Skip ⏭", callback_data: "Skip ⏭" }]] }})
    }else{
...
bot.action(['☑️ Done','Skip ⏭'], async (ctx) => {
  ...
  await ctx.replyWithMarkdown('*👋Hello '+ctx.from.first_name+' || Here is your statics*\n\n*📊 Airdrop Balance: '+sum+' $'+bot_cur+'*\n\n🙇‍♀️* Your personal referral link: https://t.me/'+data.bot_username.split('@')[1]+'?start=' + ctx.from.id + '*\n\n*👨‍✈️ Telegram: **'+ctx.from.first_name+'*\n*💬 YouTube Link: **'+fb+'*\n*🏦 Wallet Address: *`'+address+'`\n\n‼️ If your submitted data wrong then you can restart the bot and resubmit the data again by clicking /start before airdrop end.', { disable_web_page_preview: true, reply_markup: { keyboard: [['📊 Statistics', '📎 Referral Link'], ['📤 Withdraw 📤'],['‼️PerfectWorld Info ‼️']], resize_keyboard: true } }).then((m) => {
    console.log(m.message_id)
    var msid = m.message_id
    db.collection('balance').updateOne({userId: ctx.from.id}, {$set: {msid: msid}}, {upsert: true})
   })
...
bot.hears('‼️PerfectWorld Info ‼️', async (ctx) => {
    ctx.deleteMessage()
   
  console.log(ctx)
  ctx.replyWithMarkdown(''+data.details+'',{disable_web_page_preview: true, reply_markup: { keyboard: [['📊 Statistics', '📎 Referral Link'], ['📤 Withdraw 📤'],['‼️PerfectWorld Info ‼️']], resize_keyboard: true } })
  })
...
let tgData = await bot.telegram.getChatMember("@unicorn_drops", ctx.from.id) // user`s status on the channel
    let subscribed
